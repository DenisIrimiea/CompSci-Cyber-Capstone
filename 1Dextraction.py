...
def refine_line(img, x0, y0, x1, y1, width=1,
    ...
    for o in range(-search_radius, search_radius + 1):
        ...
        # look closer to the first-order end (80% of the way)
...
def branch_score(data, x0, y0, i1_box, j1_box,
    ...
    # No candidate on this side
    if (i1_box is None) or (j1_box is None):
        return -np.inf

    # Candidate first-order tile center
    ...
    # Refine line to hug streak, zeroth -> candidate
    ...
    # Zeroth position along the line
    vx = x0_ref - x_start
    vy = y0_ref - y_start
    t_zero = vx * ux_ref + vy * uy_ref
    i_zero = int(np.argmin(np.abs(t - t_zero)))

    # First-order region: after zeroth + margin
    ...
    # Score = width * mean_excess_flux for best segment
    best_score = -np.inf
    for seg in segments:
        if seg.size == 0:
            continue
        seg_width = seg.size                  # width in samples (∝ pixels)
